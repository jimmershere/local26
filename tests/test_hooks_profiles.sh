...
(
  ...
  out="$($repo_root/bin/local26 profile create prod 2>&1)"
  printf '%s\n' "$out" | grep -q "Created profile:" || { printf 'FAIL: missing create confirmation\n'; exit 1; }
  test -f .local26/profiles/prod.yaml || { printf 'FAIL: profile file was not created\n'; exit 1; }

  out="$($repo_root/bin/local26 profiles 2>&1)"
  printf '%s\n' "$out" | grep -q "Local-26 profiles" || { printf 'FAIL: missing profiles header\n'; exit 1; }
  printf '%s\n' "$out" | grep -q '^prod$' || { printf 'FAIL: missing created profile in listing\n'; exit 1; }

  duplicate_out="$tmpdir/profile-duplicate.out"
  if "$repo_root/bin/local26" profile create prod >"$duplicate_out" 2>&1; then
...
(
  ...
  cat > .local26/hooks/pre-deploy.sh <<'HOOK'
#!/usr/bin/env bash
exit 0
HOOK
  chmod +x .local26/hooks/pre-deploy.sh

  out="$($repo_root/bin/local26 hooks 2>&1)"
  printf '%s\n' "$out" | grep -q "Local-26 hooks" || { printf 'FAIL: missing hooks header\n'; exit 1; }
