...
PY
  ...
  # stub: records calls; supports SERAF_FAIL_STEP env (1-based index)
...
RSYNC
  ...
  local run_dir
  run_dir="$(ls -1 "$tmpdir/.seraf/runs" | LC_ALL=C sort | tail -n1)"
  local run_json="$tmpdir/.seraf/runs/$run_dir/run.json"
...
test_rollback_on_failure() {
  ...
  local fp
  fp="$(sha256sum "$tmpdir/.seraf/config.ini" | awk '{print $1}')"
  python3 - <<'PY' "$tmpdir/.seraf/plans/test.plan.json" "$fp" "$rb_log"
import json, sys
...
RSYNC
  ...
  local run_dir
  run_dir="$(ls -1 "$tmpdir/.seraf/runs" | LC_ALL=C sort | tail -n1)"
  local rollback_log="$tmpdir/.seraf/runs/$run_dir/rollback.log"
...
RSYNC
  ...
  local call_count
  call_count="$(wc -l < "$SERAF_STUB_LOG" | tr -d ' ')"
...
RSYNC
  ...
  [ ! -f "$tmpdir/.seraf/runs/$run_dir/rollback.log" ] || {
