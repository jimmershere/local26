...
from local26.plan_integrity import config_fingerprint, is_valid_config_fingerprint
...
def test_run_plan_writes_config_fingerprint(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert rc == 0
    assert plan["config_fingerprint"] == config_fingerprint(tmp_path / ".local26" / "config.ini")
