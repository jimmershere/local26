...
from local26.commands.deploy import _resolve_plan_path, parse_hosts_file, run_check, run_deploy
...
def test_resolve_latest_plan_path(tmp_path: Path) -> None:
    plans_dir = tmp_path / ".local26" / "plans"
...
def test_run_deploy_dry_run_writes_run_and_state(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    run_files = list((tmp_path / ".local26" / "runs").glob("*/run.json"))
    ...
    state = json.loads((tmp_path / ".local26" / "state" / "web.json").read_text(encoding="utf-8"))
...
def test_run_deploy_failure_records_stderr(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert not (tmp_path / ".local26" / "state" / "web.json").exists()
...
def test_check_warns_for_stale_config_fingerprint(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert rc == 0
    assert "[warn] plan config_fingerprint does not match current config .local26/config.ini" in out
...
def test_check_latest_plan(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert rc == 0
    assert ".local26/plans/20260501T020202Z-p1.plan.json" in out
...
def test_check_wrong_kind(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    p.write_text(json.dumps({
        "kind": "report", "mode": "deploy", "schema": "local26.plan.v0.1",
        "plan_id": "x", "scopes": [],
    }), encoding="utf-8")
    rc = run_check(plan=str(p))
    out = capsys.readouterr().out
...
def test_deploy_latest_plan_file(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert rc == 0
    assert ".local26/plans/20260501T020202Z-new.plan.json" in out
...
def test_deploy_remote_cmd_uses_ssh_target(tmp_path: Path, monkeypatch, capsys) -> None:
    import local26.commands.deploy as deploy_module
...
def test_deploy_multi_host_fail_fast(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    payload = {
        "schema": "local26.plan.v0.1", "kind": "plan", "mode": "deploy",
        "plan_id": "ff1",
        "scopes": [{"scope": "web", "steps": steps}],
    }
    plan_path.write_text(json.dumps(payload), encoding="utf-8")
