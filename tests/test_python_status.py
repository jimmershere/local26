...
from local26.commands.status import load_status_record, render_status


def test_status_no_runs(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".local26" / "runs").mkdir(parents=True)
    out = render_status()
    assert "Local-26 status" in out
...
def test_status_reads_latest_run(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = tmp_path / ".local26" / "runs" / "20260101T000000Z-99999"
...
def test_status_reads_home_state(tmp_path: Path, monkeypatch) -> None:
    home = tmp_path / "home"
    (home / ".local26").mkdir(parents=True)
    (home / ".local26" / "state.json").write_text(json.dumps({
