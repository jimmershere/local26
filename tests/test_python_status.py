...
from seraf.commands.status import load_status_record, render_status
...
def test_status_reads_latest_run(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    run_dir = tmp_path / ".seraf" / "runs" / "20260101T000000Z-99999"
...
def test_status_reads_home_state(tmp_path: Path, monkeypatch) -> None:
    home = tmp_path / "home"
    (home / ".seraf").mkdir(parents=True)
    (home / ".seraf" / "state.json").write_text(json.dumps({
