...
mkdir -p "$tmpdir/.local26/plans" "$tmpdir/.local26/runs" "$tmpdir/.local26/state" "$tmpdir/stubs"
...
(
  cd "$tmpdir"
  PATH="$tmpdir/stubs:$PATH" "$repo_root/bin/local26" deploy --plan "$tmpdir/.local26/plans/policy.plan.json" --scope web >"$tmpdir/web.out"
)

grep -q 'Deploy finished cleanly' "$tmpdir/web.out"
grep -q '^rsync ' "$LOCAL26_STUB_LOG"

run_dir="$(ls -1 "$tmpdir/.local26/runs" | sort | tail -n1)"
[ "$(stat -c '%a' "$tmpdir/.local26/runs/$run_dir")" = "700" ]
[ "$(stat -c '%a' "$tmpdir/.local26/runs/$run_dir/run.json")" = "600" ]
[ "$(stat -c '%a' "$tmpdir/.local26/runs/$run_dir/run.log")" = "600" ]
[ "$(stat -c '%a' "$tmpdir/.local26/state/web.json")" = "600" ]

spoofed_actor="$(LOGNAME=spoofed USER=spoofed PYTHONPATH="$repo_root/src" python3 - <<'PY'
from local26.policy import current_actor
...
(
  cd "$tmpdir"
  PATH="$tmpdir/stubs:$PATH" "$repo_root/bin/local26" deploy --plan "$tmpdir/.local26/plans/policy.plan.json" --scope admin --check >"$tmpdir/check-admin.out" 2>"$tmpdir/check-admin.err"
)
check_admin_rc=$?
set -e
...
(
  cd "$tmpdir"
  PATH="$tmpdir/stubs:$PATH" "$repo_root/bin/local26" deploy --plan "$tmpdir/.local26/plans/policy.plan.json" --scope admin >"$tmpdir/admin.out" 2>"$tmpdir/admin.err"
)
admin_rc=$?
set -e

[ "$admin_rc" -ne 0 ]
grep -q 'Local-26 deploy blocked by access-control policy' "$tmpdir/admin.out"
grep -q 'remote_cmd steps require access.allow_remote_cmd = true' "$tmpdir/admin.out"

(
  cd "$tmpdir"
  "$repo_root/bin/local26" compliance report >"$tmpdir/compliance.out"
)

grep -q 'Local-26 compliance report' "$tmpdir/compliance.out"
...
(
  cd "$tmpdir"
  "$repo_root/bin/local26" compliance report >"$tmpdir/compliance-bad.out" 2>"$tmpdir/compliance-bad.err"
)
bad_compliance_rc=$?
set -e
