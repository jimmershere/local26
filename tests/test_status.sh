...
(
  cd "$tmpdir"
  mkdir -p .local26/runs
  out="$("$repo_root/bin/local26" status 2>&1)"
  printf '%s\n' "$out" | grep -q "Local-26 status" || { printf 'FAIL: missing header\n'; exit 1; }
...
(
  cd "$tmpdir"
  local_run_dir=".local26/runs/20260101T000000Z-99999"
...
(
  cd "$tmpdir"
  local_run_dir2=".local26/runs/20260102T000000Z-88888"
...
(
  cd "$tmpdir"
  mkdir -p "${HOME}/.local26"
  state_file="${HOME}/.local26/state.json"
  cat > "$state_file" <<'JSON'
