...
(
  cd "$tmpdir"
  mkdir -p .seraf/runs
  out="$("$repo_root/bin/seraf" status 2>&1)"
  printf '%s\n' "$out" | grep -q "seraf status" || { printf 'FAIL: missing header\n'; exit 1; }
  printf '%s\n' "$out" | grep -q "(none)" || { printf 'FAIL: expected (none) for active runs\n'; exit 1; }
  printf '%s\n' "$out" | grep -q "(no runs found)" || { printf 'FAIL: expected no-runs message\n'; exit 1; }
)

# ── Test 2: status reads from last run.json ──────────────────────────────────
(
  cd "$tmpdir"
  local_run_dir=".seraf/runs/20260101T000000Z-99999"
...
(
  cd "$tmpdir"
  local_run_dir2=".seraf/runs/20260102T000000Z-88888"
...
(
  cd "$tmpdir"
  mkdir -p "${HOME}/.seraf"
  state_file="${HOME}/.seraf/state.json"
  cat > "$state_file" <<'JSON'
