...
init_out="$(mktemp "${tmpdir}/local26-test-out.XXXXXX")"
init_err="$(mktemp "${tmpdir}/local26-test-err.XXXXXX")"

(
  cd "$tmpdir"
  "$repo_root/bin/local26" init --force --project demo >"$init_out" 2>"$init_err"
)

test -f "${tmpdir}/.local26/config.ini"
grep -q '^default_scope = web$' "${tmpdir}/.local26/config.ini"
grep -q '^\[scope "web"\]' "${tmpdir}/.local26/config.ini"
grep -q '^\[scope "api"\]' "${tmpdir}/.local26/config.ini"
grep -q '^servers = app1,app2,app3$' "${tmpdir}/.local26/config.ini"
grep -q '^backup = true$' "${tmpdir}/.local26/config.ini"
grep -q "legacy section \[api\] has invalid backup value 'maybe'; expected true/false, ignoring" "$init_err"

web_line="$(grep -n '^\[scope "web"\]' "${tmpdir}/.local26/config.ini" | cut -d: -f1)"
api_line="$(grep -n '^\[scope "api"\]' "${tmpdir}/.local26/config.ini" | cut -d: -f1)"
if [ "$web_line" -ge "$api_line" ]; then
  echo "expected web scope before api scope" >&2
  exit 1
fi

if grep -q '^source_dir = /ignored$' "${tmpdir}/.local26/config.ini"; then
  echo "duplicate scope should keep first declaration" >&2
  exit 1
fi

api_scope_block="$(sed -n '/^\[scope "api"\]/,/^\[scope "/p' "${tmpdir}/.local26/config.ini")"
...
grep -Eq '"last_success"[[:space:]]*:[[:space:]]*"2024-02-03T04:05:06Z"' "${tmpdir}/.local26/state/web.json"
grep -Eq '"last_success"[[:space:]]*:[[:space:]]*null' "${tmpdir}/.local26/state/api.json"

second_out="$(mktemp "${tmpdir}/local26-test-out-2.XXXXXX")"
second_err="$(mktemp "${tmpdir}/local26-test-err-2.XXXXXX")"

set +e
(
  cd "$tmpdir"
  "$repo_root/bin/local26" init >"$second_out" 2>"$second_err"
)
second_rc=$?
set -e
...
missing_out="$(mktemp "${missing_tmpdir}/local26-test-out-3.XXXXXX")"
missing_err="$(mktemp "${missing_tmpdir}/local26-test-err-3.XXXXXX")"
set +e
(
  cd "$missing_tmpdir"
  "$repo_root/bin/local26" init >"$missing_out" 2>"$missing_err"
)
missing_rc=$?
set -e
