...
from local81.commands.doctor import run_doctor
from local81.commands.hooks import run_hooks
from local81.commands.profiles import run_profile_create, run_profiles
...
def test_run_hooks_lists_statuses(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    hooks_dir = tmp_path / ".local81" / "hooks"
...
def test_pre_deploy_hook_abort(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.chdir(tmp_path)
    _write_config(tmp_path / ".local81" / "config.ini")
    pre = tmp_path / ".local81" / "hooks" / "pre-deploy.sh"
...
def test_post_deploy_hook_warns_only(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.chdir(tmp_path)
    _write_config(tmp_path / ".local81" / "config.ini")
    post = tmp_path / ".local81" / "hooks" / "post-deploy.sh"
...
def test_profiles_create_and_list(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert rc == 0
    assert (tmp_path / ".local81" / "profiles" / "prod.yaml").exists()
    rc = run_profiles()
    out = capsys.readouterr().out
    assert rc == 0
    assert "prod" in out
...
def test_profile_merge_changes_scope_and_notifications(tmp_path: Path) -> None:
    _write_config(tmp_path / ".local81" / "config.ini")
    profile = tmp_path / ".local81" / "profiles" / "prod.yaml"
    profile.parent.mkdir(parents=True)
    profile.write_text(
        "defaults:\n  rsync_opts: -aP\nnotifications:\n  notify_on_success: true\nscopes:\n  web:\n    target_dir: /srv/prod\n    servers: [prod1, prod2]\n",
        encoding="utf-8",
    )
    cfg = load_config(tmp_path / ".local81" / "config.ini", profile="prod")
...
def test_doctor_validates_profile(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.chdir(tmp_path)
    _write_config(tmp_path / ".local81" / "config.ini")
    prof = tmp_path / ".local81" / "profiles" / "prod.yaml"
...
def test_deploy_notifications_success_forced(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("local81.notifications.post_json", fake_post_json)
...
def test_deploy_notifications_quiet(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_plan(plan)
    calls = []
    monkeypatch.setattr("local81.notifications.post_json", lambda *args, **kwargs: calls.append(args))
...
def test_timeout_alert_sends_warning(tmp_path: Path, monkeypatch) -> None:
    ...
    calls = []
    monkeypatch.setattr("local81.notifications.post_json", lambda url, payload, timeout=10: calls.append(payload))
