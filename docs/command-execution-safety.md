...
Local-26 currently keeps deployment behavior stable while reducing command execution risk through inventory, diagnostics, and security checks.
## Approved execution paths
The following Python execution paths are known and intentional:
- `src/local26/commands/deploy.py` `_run_shell`: executes plan step `cmd` strings through `bash -lc`. This is the only approved local shell interpreter site because existing plan steps are stored as shell command strings and may include pipes, redirection, rollback snippets, or generated `ssh`/`rsync` commands.
- `src/local26/commands/deploy.py` `_run_remote`: executes `ssh <host> <command>` with argv form for `remote_cmd` steps.
- `src/local26/hooks.py` `run_hook`: executes owner-controlled hook files under `.local26/hooks` with `bash <path>`.
- `src/local26/commands/plan.py` `_git_checkout`: executes `git` with argv form only for configured git workspaces.
- `src/local26/commands/pull.py`, `src/local26/commands/pull_logs.py`, `src/local26/commands/diag.py`, and `src/local26/notifications.py`: execute specific tools with argv form and no `shell=True`.
