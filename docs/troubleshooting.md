...
## `local26 db doctor` says no targets matched
Add `[database "NAME"]` sections to `.local26/config.ini` or a `databases:` mapping in `.local26/config.yaml`. Make sure each target has `engine = oracle19c`, `engine = postgres17`, or `engine = sqlite`, and that `enabled` is not set to `false`.

## `local26 db` reports missing Oracle or PostgreSQL tools
...
## `local26 db backup` did not create a SQLite backup
SQLite backups require both an explicit destination and execution approval:

```bash
local26 db backup --target local-cache --backup-path .local26/db/local-cache.bak --execute
```

Without `--execute`, Local-26 only writes a backup plan.

## `local26 compliance report` exits nonzero
...
```bash
local26 compliance report --fail-on never
```
