...
def _find_run(run_id: str, runs_dir: str = ".seraf/runs") -> Path | None:
...
def render_run_log(run_id: str, *, runs_dir: str = ".seraf/runs") -> str:
    """Render full output for a single run."""
    run_file = _find_run(run_id, runs_dir)
    if not run_file:
        return f"Run not found: {run_id}\nCheck 'seraf history' for available run IDs."
