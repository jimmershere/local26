...
def run_pull_logs(*, settings: str = "./settings.cfg", hosts: str | None = None,
    ...
    dest_value = dest or settings_data.get("log_dest_dir") or ".seraf/pulled-logs"
    ...
    if not hosts_csv:
        print("seraf: no log hosts configured; set log_hosts in settings.cfg or pass --hosts", file=sys.stderr)
        return 1

    if not any(path for path in component_paths.values()):
        print("seraf: no log paths configured; define jboss/apache/engin/smartxfr paths in settings.cfg or on command line", file=sys.stderr)
        return 1
    ...
    for host in _iter_hosts(hosts_csv):
        ...
        for component, remote_path in component_paths.items():
            ...
            if proc.returncode == 0:
                ...
                print(f"seraf: warning: failed to pull {component} logs from {host}:{remote_path}", file=sys.stderr)
                failed_count += 1
