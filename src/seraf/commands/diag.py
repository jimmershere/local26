...
def run_diag(*, project: str | None = None, hosts: str | None = None,
    ...
    if remote_cmd and pid:
        print("seraf: use either --remote-cmd or --pid/--diag-type, not both")
        return 2

    if not remote_cmd:
        if not pid:
            print("seraf: --pid is required unless --remote-cmd is provided")
            return 2
        try:
    ...
    if not host_list:
        print("seraf: no hosts provided; use --hosts CSV or --project NAME")
        return 2

    output_dir = Path(out_dir) if out_dir else Path(".seraf") / "diag" / "latest"
