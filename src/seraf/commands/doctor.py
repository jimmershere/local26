...
from seraf.profiles import list_profiles
...
class CheckResult:
    ...
    def render(self) -> str:
        icon = {"PASS": "[ok]", "WARN": "[warn]", "FAIL": "[fail]"}[self.level]
...
def run_doctor(plan: str | None = None, profile: str | None = None) -> int:
    ...
    for title, bucket in (("Ready", passes), ("Needs attention", warns), ("Blocking issues", fails)):
        ...
        print()
    if fails:
        print("Doctor found blocking issues. Fix those first, then run 'seraf doctor' again.")
        return 1
    if warns:
