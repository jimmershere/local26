...
from seraf.hooks import run_hook
from seraf.notifications import NotificationEvent, notify_all
from seraf.state import load_scope_state
...
def _update_scope_state(scope_name: str, *, plan_id: str | None, run_id: str, rc: int, deployed_files: int) -> None:
    ...
    _write_json(Path(".seraf") / "state" / f"{scope_name}.json", payload)
...
def run_deploy(*, plan: str, scope: str | None = None, max_parallel: int = 1,
    ...
    if not scopes:
        print("Seraf deploy did not find any matching scopes in that plan.")
        return 1
    notifications_cfg = {}
    if Path(".seraf/config.ini").exists():
        notifications_cfg = load_config(profile=profile).notifications
    hosts = None
    if hosts_file:
        hosts = parse_hosts_file(hosts_file)
        if not hosts:
            print(f"Seraf deploy found no hosts in {hosts_file}")
            return 1
    ...
    hook_env = {
        **{k: v for k, v in __import__('os').environ.items()},
        "SERAF_PLAN": str(plan_path),
        "SERAF_RUN_ID": run_id,
        "SERAF_PROFILE": profile or "",
    }
    ...
    payload: dict = {"schema": "seraf.run.v0.1", "run_id": run_id, "plan_id": plan_data.get("plan_id"), "started_at": all_steps[0]["started_at"] if all_steps else _now_iso(), "finished_at": _now_iso(), "rc": overall_rc, "dry_run": dry_run, "steps": all_steps, "profile": profile, "notification_warnings": notification_warnings}
    if host_results:
    ...
    post_rc, post_out, post_err = run_hook("post-deploy.sh", env={**hook_env, "SERAF_DEPLOY_RC": str(overall_rc)})
    if post_out:
        print(post_out)
    if post_err:
        print(post_err)
