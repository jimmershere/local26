...
class Oracle19cAdapter(DatabaseAdapter):
    ...
    def command_plans(self, action: str, *, execute: bool = False) -> list[CommandPlan]:
        ...
        plans = [
            ...
            CommandPlan("sqlplus-instance", ["sqlplus", "-L", "/nolog", "@local26_oracle_health.sql"], "Run read-only Oracle health SQL"),
        ]
        if action in {"backup", "doctor", "report"}:
            plans.append(CommandPlan("rman-validate", ["rman", "target", "/", "cmdfile=local26_rman_validate.rcv"], "Validate RMAN backup/readiness", True))
            plans.append(CommandPlan("datapump-export-plan", ["expdp", f"service_name={service}", "parfile=local26_export.par"], "Plan Data Pump export", True))
