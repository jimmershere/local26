...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="local26",
        description="Local-26 operator CLI for deploy planning, diagnostics, compliance checks, and database readiness.",
    )
    parser.add_argument("--profile", default=None, help="Apply a profile overlay from .local26/profiles/ when the command supports it.")
    sub = parser.add_subparsers(dest="command", required=True, metavar="COMMAND")

    init = sub.add_parser("init", help="Create or import a .local26 project config.")
    ...
    for db_command in ("doctor", "inventory", "tools", "monitor", "diag", "backup", "report", "audit"):
        ...
        db_parser.add_argument("--config", default=None, help="Config file to read; defaults to .local26/config.ini with YAML fallback.")
        ...
        db_parser.add_argument("--output-dir", default=None, help="Artifact root; defaults to .local26/db.")
