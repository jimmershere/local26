...
def run_guided_interview(*, inp: IO[str] = sys.stdin, out: TextIO = sys.stdout) -> GuidedAnswers | None:
    ...
    out.write("\nWelcome to Local-26 guided setup.\n")
    ...
    _section("Deployment source", "Point Local-26 at the local files you actually want to ship.", out=out)
    ...
    if source_path.exists():
        ...
        if not keep_going:
            answers.source_dir = _prompt("Okay, what local directory should Local-26 deploy from instead?", inp=inp, out=out)
            if not answers.source_dir:
                out.write("Stopping here so we do not write a broken config.\n")
                return None
...
def generate_config(answers: GuidedAnswers) -> str:
    ...
    local26 = payload["local26"]
    routing = payload["routing"]
    scope = payload["scopes"][answers.scope_name]
    lines = [
        "[local26]",
        f"version = {local26['version']}",
        f"project = {local26['project']}",
        f"default_scope = {local26['default_scope']}",
        f"state_dir = {local26['state_dir']}",
        f"plans_dir = {local26['plans_dir']}",
        f"runs_dir = {local26['runs_dir']}",
        f"logs_dir = {local26['logs_dir']}",
        f"lock_file = {local26['lock_file']}",
        f"require_plan_for_deploy = {'true' if local26['require_plan_for_deploy'] else 'false'}",
        f"fail_fast = {'true' if local26['fail_fast'] else 'false'}",
        f"max_parallel = {local26['max_parallel']}",
        f"shell = {local26['shell']}",
        "",
        "[tools]",
        "ssh = /usr/bin/ssh",
        "rsync = /usr/bin/rsync",
...
def preview_config(config_text: str, *, out: TextIO = sys.stdout) -> None:
    _section("Config preview", "This is the INI form Local-26 uses today. A YAML mirror will be written too.", out=out)
...
def _write_project_files(answers: GuidedAnswers, config_text: str, config_yaml: str, *, out: TextIO = sys.stdout) -> int:
    local26_dir = Path(".local26")
    config_ini_path = local26_dir / "config.ini"
    config_yaml_path = local26_dir / "config.yaml"
    local26_dir.mkdir(parents=True, exist_ok=True)
    for sub in ("state", "plans", "runs", "logs"):
        (local26_dir / sub).mkdir(parents=True, exist_ok=True)
    config_ini_path.write_text(config_text, encoding="utf-8")
    config_yaml_path.write_text(config_yaml, encoding="utf-8")

    state = {
        "schema": "local26.state.v0.1",
    ...
    state_file = local26_dir / "state" / f"{answers.scope_name}.json"
    ...
    out.write(f"Local-26 is ready for project '{answers.project}' with default scope '{answers.scope_name}'.\n")
    out.write("Next good steps: run 'local26 doctor', then 'local26 plan --summary'.\n")
    return 0
...
def run_guided(*, force: bool = False, inp: IO[str] = sys.stdin, out: TextIO = sys.stdout) -> int:
    ...
    if (config_ini_path.exists() or config_yaml_path.exists()) and not force:
        out.write(f"local26: {config_ini_path} or {config_yaml_path} already exists; rerun with --force\n")
        return 1
