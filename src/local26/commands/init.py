...
def _warn(message: str) -> None:
    print(f"local26: warning: {message}", file=sys.stderr)


def _err(message: str) -> None:
    print(f"local26: {message}", file=sys.stderr)
...
def _modern_config_info(path: Path) -> tuple[bool, bool, list[str]]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if _SECTION_RE.match(stripped):
            in_local26 = False
            continue
        if in_local26 and "=" in stripped:
            key, value = stripped.split("=", 1)
            if key.strip() == "version" and value.strip() == "0.1":
                version_ok = True
    return has_local26, version_ok, scopes
...
def _upsert_modern_project(path: Path, project: str) -> None:
    ...
    for line in lines:
        ...
        if stripped in {"[local26]", "[seraf]"}:
            ...
            out.append("[local26]")
            continue
        if in_local26 and _SECTION_RE.match(stripped):
            if not wrote_project:
                out.append(f"project = {project}")
                wrote_project = True
            in_local26 = False
        if in_local26 and "=" in line:
            ...
            if key.strip() == "project":
                ...
                continue
        out.append(line)
    if saw_local26 and in_local26 and not wrote_project:
        out.append(f"project = {project}")
    path.write_text("\n".join(out) + "\n", encoding="utf-8")
...
def run_init(*, import_path: str | None = None, force: bool = False, project: str | None = None) -> int:
    ...
    config_path = local26_dir / 'config.ini'
    ...
    if not legacy.exists():
        ...
        return 1
    has_local26, version_ok, modern_scopes = _modern_config_info(legacy)
    if has_local26 or modern_scopes:
        if not has_local26:
            _err("modern config import failed: missing [local26] section")
            return 1
        if not version_ok:
            _err("modern config import failed: [local26] version must be 0.1")
            return 1
        if not modern_scopes:
            _err('modern config import failed: missing [scope "NAME"] sections')
            return 1
        for path in (local26_dir, state_dir, plans_dir, runs_dir, logs_dir):
            _mkdir_private(path)
        shutil.copyfile(legacy, config_path)
        config_text = config_path.read_text(encoding="utf-8").replace("[seraf]", "[local26]")
        ...
        print("Local-26 project initialized")
        print("========================")
        print(f"Imported {len(modern_scopes)} scope(s) from {legacy}.")
        print(f"Config written to: {config_path}")
        print("Next good steps: run 'local26 doctor', then 'local26 plan --summary'.")
        return 0
