...
def run_pull(*, scope: str | None = None, hosts: str | None = None,
    ...
    if not selected_scopes:
        print("local81: no matching scopes found", file=sys.stderr)
        return 1
    ...
    for scope_config in selected_scopes:
        ...
        if not scope_config.source_dir or not scope_config.target_dir or not effective_hosts:
            print(f"local81: warning: scope={scope_config.name} skipped (needs source_dir, target_dir, and servers/--hosts)", file=sys.stderr)
            continue
        ...
        for host in effective_hosts:
            ...
            if proc.returncode == 0:
                ...
                print(f"local81: warning: scope={scope_config.name} host={host} failed", file=sys.stderr)
                failed_count += 1
