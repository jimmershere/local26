...
def render_inventory(report: InventoryReport, output_format: str) -> str:
    ...
    if output_format == "markdown":
        lines = ["# Local-81 compliance inventory", f"- Root: `{report.root}`", f"- Items: {len(report.items)}", ""]
        if report.items:
            lines.append("## Items")
...
def render_harden_plan(plan: HardenPlan, output_format: str) -> str:
    ...
    if output_format == "markdown":
        lines = ["# Local-81 hardening plan", f"Root: `{plan.root}`", "", "All actions are advisory and non-mutating.", ""]
        if plan.items:
        ...
        return "\n".join(lines).rstrip()
    lines = ["Local-81 hardening plan", "=======================", f"Root: {plan.root}", "Actions are advisory and non-mutating.", ""]
    if plan.items:
        for item in plan.items:
