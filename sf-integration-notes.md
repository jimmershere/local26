...
- 2026-04-21: Migrated the next missing Python CLI handler, `pull`, from the shell wrapper into `src/seraf/commands/pull.py`.
- Added CLI wiring for `seraf pull --scope/--hosts/--rsync-opts/--dry-run` in `src/seraf/cli.py`.
...
- 2026-04-22: Migrated the next missing Python CLI handler, `diag`, from the shell wrapper into `src/seraf/commands/diag.py`.
- Added CLI wiring for `seraf diag --project/--hosts/--diag-type/--pid/--duration/--remote-cmd/--out-dir/--ssh-user/--include-disabled/--dry-run` in `src/seraf/cli.py` and updated `bin/seraf` to dispatch `diag` through the Python entrypoint.
...
- 2026-04-23: Migrated the next missing Python CLI handler, `pull-logs`, from the shell wrapper into `src/seraf/commands/pull_logs.py`.
- Added CLI wiring for `seraf pull-logs --settings/--hosts/--dest/--jboss-path/--apache-path/--engin-path/--smartxfr-path` in `src/seraf/cli.py` and updated `bin/seraf` to dispatch `pull-logs` through the Python entrypoint.
...
- 2026-04-25: The next backlog item was already implemented in Python, so I focused on the missing `plan --summary` integration coverage and restored the documented summary contract in `src/seraf/commands/plan.py`.
...
- Updated `bin/seraf` usage and Python dispatch so `seraf diff PLAN_A PLAN_B` now resolves through the Python CLI like the other migrated commands.
...
- Verified with `PYTHONPATH=src pytest -q tests/test_python_diff.py` and a `./bin/seraf diff <planA> <planB>` smoke test.
...
- Updated `bin/seraf` usage and Python dispatch so `seraf history`, `seraf logs`, `seraf hooks`, `seraf profiles`, and `seraf profile create` resolve through the Python CLI consistently.
...
- Added `tests/test_hooks_profiles.sh` to verify `bin/seraf` dispatch for profile scaffolding, duplicate-profile failure handling, profile listing, and hook status rendering.
...
- Added `tests/test_init_guided.sh` to verify `bin/seraf help` advertises `init --guided` and that the interactive guided flow writes a usable `.seraf/config.ini` plus initial state through the shell entrypoint.
- Added CLI dispatch coverage in `tests/test_python_cli_dispatch.py` for `seraf init --guided --force` so the Python path stays pinned during future wrapper migrations.
...
- 2026-05-01: The next unimplemented deploy surface was `seraf deploy --latest`, and the Python deploy path still had shell-parity gaps around run artifacts and `remote_cmd` host execution.
- Added `--latest` parser/handler support so deploy/check can resolve the newest `.seraf/plans/*.plan.json` without an explicit `--plan` path.
