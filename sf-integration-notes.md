...
- 2026-04-21: Migrated the next missing Python CLI handler, `pull`, from the shell wrapper into `src/local81/commands/pull.py`.
- Added CLI wiring for `local81 pull --scope/--hosts/--rsync-opts/--dry-run` in `src/local81/cli.py`.
...
- 2026-04-22: Migrated the next missing Python CLI handler, `diag`, from the shell wrapper into `src/local81/commands/diag.py`.
- Added CLI wiring for `local81 diag --project/--hosts/--diag-type/--pid/--duration/--remote-cmd/--out-dir/--ssh-user/--include-disabled/--dry-run` in `src/local81/cli.py` and updated `bin/local81` to dispatch `diag` through the Python entrypoint.
...
- 2026-04-23: Migrated the next missing Python CLI handler, `pull-logs`, from the shell wrapper into `src/local81/commands/pull_logs.py`.
- Added CLI wiring for `local81 pull-logs --settings/--hosts/--dest/--jboss-path/--apache-path/--engin-path/--smartxfr-path` in `src/local81/cli.py` and updated `bin/local81` to dispatch `pull-logs` through the Python entrypoint.
...
- 2026-04-25: The next backlog item was already implemented in Python, so I focused on the missing `plan --summary` integration coverage and restored the documented summary contract in `src/local81/commands/plan.py`.
...
- Updated `bin/local81` usage and Python dispatch so `local81 diff PLAN_A PLAN_B` now resolves through the Python CLI like the other migrated commands.
...
- Verified with `PYTHONPATH=src pytest -q tests/test_python_diff.py` and a `./bin/local81 diff <planA> <planB>` smoke test.
...
- Updated `bin/local81` usage and Python dispatch so `local81 history`, `local81 logs`, `local81 hooks`, `local81 profiles`, and `local81 profile create` resolve through the Python CLI consistently.
...
- Added `tests/test_hooks_profiles.sh` to verify `bin/local81` dispatch for profile scaffolding, duplicate-profile failure handling, profile listing, and hook status rendering.
...
- Added `tests/test_init_guided.sh` to verify `bin/local81 help` advertises `init --guided` and that the interactive guided flow writes a usable `.local81/config.ini` plus initial state through the shell entrypoint.
- Added CLI dispatch coverage in `tests/test_python_cli_dispatch.py` for `local81 init --guided --force` so the Python path stays pinned during future wrapper migrations.
...
- 2026-05-01: The next unimplemented deploy surface was `local81 deploy --latest`, and the Python deploy path still had shell-parity gaps around run artifacts and `remote_cmd` host execution.
- Added `--latest` parser/handler support so deploy/check can resolve the newest `.local81/plans/*.plan.json` without an explicit `--plan` path.
